    ...
    loadData (pagination, filters = this.filters, sorter = this.sorter) {
      ...
      if ((typeof result === 'object' || typeof result === 'function') && typeof result.then === 'function') {
        result.then(r => {
          ...
          let dataList = r[this.DataName]
          // 为防止删除数据后导致页面当前页面数据长度为 0 ,自动翻页到上一页
          if (dataList.length === 0 && this.showPagination && this.localPagination.current > 1) {
          ...
          if (this.handleMethods) {
            dataList = this.handleMethods(dataList)
          }
          this.localDataSource = dataList // 返回结果中的数组数据
